...
data <- data %>% mutate(engage_score = projects_added + comment_given + likes_given + trunc(session_likes_given/med_likes) + trunc(session_comments_given/med_comments) 
...
data_original <- data
data <- subset(data_original,select = -c())
